...
| GET | `/peers?network=&window=&steps=` | The producer set over the last N blocks (50–10000, default 1000), each with block count and balance, plus totals. The walk is batched 2000 blocks at a time, so a 10,000-block window is five requests and about six seconds. Cached, because the walk is the expensive part. Producers whose balance could not be read are excluded from `totalBalance` rather than counted as zero. `steps` is a comma-separated list of shallower depths (`steps=1000,2000,5000,10000`) tallied from the SAME scan and returned under `steps` — a 10,000-block walk already contains the 1,000-block answer, so asking separately would pull 18,000 blocks to learn what 10,000 said |
